    ...
    for (sample_number in benchmark_samples){
        for (k in Ks){
            ...
            for (split_version in 0:3){
                ...
                seurat_obj <- CreateSeuratObject(counts = t(adata_train$X))
                ...
                seurat_obj <- FindNeighbors(seurat_obj, dims = 1:10, k.param=k, reduction = "pca")
                ...
                imputed_data <- t(seurat_obj@assays$RNA@counts)
